from prefect import task, flow, logger
import requests
import psycopg2
...
@task()
def upsert_page(table_name, csv_url, postgres_creds, triply_creds):
    # Load logger
...
def main_flow(
    ...
    for table_name, csv_url in table_config.items():
        upsert_page(table_name=table_name, csv_url=csv_url, triply_creds=triply_creds, postgres_creds=postgres_creds).submit()
