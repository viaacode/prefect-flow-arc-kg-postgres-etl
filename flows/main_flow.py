...
def main_flow(
    ...
    kg_view_last_flow_run_failed = check_deployment_last_flow_run_failed(
        deployment_kg_view_flow.name
    )
    # Check if the last flow run of the knowledge graph to postgres flow failed
    kg_to_postgres_last_flow_run_failed = check_deployment_last_flow_run_failed(
        deployment_arc_db_load_flow.name
    )
    # Check if the last flow run of the arc alto to json flow failed
    arc_alto_to_json_last_flow_run_failed = check_deployment_last_flow_run_failed(
        deployment_arc_alto_to_json_flow.name
    )
    # Check if the last flow run of the arc indexer flow failed
    arc_indexer_last_flow_run_failed = check_deployment_last_flow_run_failed(
        deployment_arc_indexer_flow.name
    )
    ...
    kg_view_last_modified_parameter_change = (
        change_deployment_parameters.submit(
            name=deployment_kg_view_flow.name,
            parameters={
                "or_ids": or_ids,
        ...
        and (
            ...
            or full_sync
            or deployment_kg_view_flow.full_sync
        )
        else None
    )
    ...
    kg_to_postgres_last_modified_parameter_change = (
        change_deployment_parameters.submit(
            name=deployment_arc_db_load_flow.name,
            parameters={
                "last_modified": last_modified,
        ...
        and (
            ...
            or full_sync
            or deployment_arc_db_load_flow.full_sync
        )
        else None
    )
    ...
    # Only change the last_modified parameter if the flow is active and the last flow run or its dependant flow runs did not fail or if it is a full sync
    arc_alto_to_json_last_modified_parameter_change = (
        change_deployment_parameters.submit(
            name=deployment_arc_alto_to_json_flow.name,
            parameters={
                "last_modified": last_modified,
        ...
        and (
            ...
            or full_sync
            or deployment_arc_alto_to_json_flow.full_sync
        )
        else None
    )
    ...
    # Only change the last_modified parameter if the flow is active and the last flow run or its dependant flow runs did not fail or if it is a full sync
    arc_indexer_last_modified_parameter_change = (
        change_deployment_parameters.submit(
            name=deployment_arc_indexer_flow.name,
            parameters={
                "last_modified": last_modified,
        ...
        and (
            ...
            or full_sync
            or deployment_arc_indexer_flow.full_sync
        )
        else None
    )
